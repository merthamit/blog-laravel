    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <form method="POST" enctype="multipart/form-data" action="{{ route('admin.makaleler.update', $article->id) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="category">
                        @foreach ($categories as $category)
                            <option @if ($article->getCategory->id == $category->id) selected @endif value="{{ $category->id }}">
                                {{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                @php
                    $baseUrlImage = url('/') . '/' . $article->image;
                @endphp
                <img src="{{ $baseUrlImage }}" class="img-thumbnail" width="200" height="400" />
                <div class="form-group">
                ...
                <div class="form-group">
                    <button class="btn btn-primary" type="submit">Makaleyi Güncelle</button>
                </div>
            </form>
        </div>
    </div>
